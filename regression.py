...
def gdp_histogram(df):
    ...
    # make a histogram
    plt.hist(df['GDP'], bins=23, color='blue', edgecolor='black', alpha=0.5)
...
def commercial_and_industrial_loans_histogram(df):
    ...
    # add the title and axis labels
    plt.title(
        'Simple Histogram of Commercial and Industrial Loans(% change from previous quarter)')
...
def univraite_regression(df, dependent, fixed_factor):
    ...
    # make a new table
    anova_table = pd.DataFrame(index=['Corrected Model', 'Hyperten', 'Sex', 'Hyperten * Sex', 'Error', 'Corrected Total'], columns=[
        'df', 'sum_sq', 'mean_sq', 'F', 'sig.'])

    # get the sum_sq, df, F, PR(>F) from the result
    for i in range(1, 5):
        anova_table.iloc[i, 0] = result.iloc[i-1, 0]
        anova_table.iloc[i, 1] = result.iloc[i-1, 1]
        anova_table.iloc[i, 2] = result.iloc[i-1, 2]
        anova_table.iloc[i, 3] = result.iloc[i-1, 3]
        anova_table.iloc[i, 4] = result.iloc[i-1, 4]

    # calculate the Sum of Squares for the model
    # cite from : https://www.statology.org/sst-ssr-sse-in-python/
    corrected_total_sr = 0
    for i in range(len(dataframe)):
        corrected_total_sr += (dataframe['time_chd']
                               [i] - dataframe['time_chd'].mean()) ** 2

    corrected_model_sr = corrected_total_sr - result.iloc[3, 1]

    print("corrected_model_sr", corrected_model_sr)
    anova_table.iloc[0, 1] = corrected_model_sr

    # [0,0] is the sum of fixed_factor
    anova_table.iloc[0, 0] = anova_table.iloc[1, 0] + \
        anova_table.iloc[2, 0] + anova_table.iloc[3, 0]

    anova_table.iloc[0, 2] = corrected_model_sr / \
        anova_table.iloc[0, 0]

    # find the f value of anova table
    anova_table.iloc[0, 3] = anova_table.iloc[0, 2] / \
        anova_table.iloc[3, 2]

    # find the f value of anova table
    anova_table.iloc[0, 3] = anova_table.iloc[0, 2] / \
        anova_table.iloc[4, 2]

    # p value calculation
    p = 1 - stats.f.cdf(anova_table.iloc[0, 3],
                        anova_table.iloc[0, 0], anova_table.iloc[4, 0])

    # save the p value
    anova_table.iloc[0, 4] = p

    # put the total sum of square
    anova_table.iloc[5, 1] = corrected_total_sr

    # put the df of total sum of square
    anova_table.iloc[5, 0] = len(dataframe) - 1
